...
def bigrams_count_to_probabilities(
    ...
    # TODO
    num = smooth_factor
    den = smooth_factor * bigram_counts.shape[0]

    bigram_probs = torch.empty(bigram_counts.shape[0], bigram_counts.shape[1])
    probs = torch.sum(bigram_counts, dim = 1)

    for i in range(bigram_counts.shape[0]):
        for j in range(bigram_counts.shape[1]):
            bigram_probs[i][j] = (bigram_counts[i][j] + num)/(probs[i] + den)
...
def calculate_neg_mean_log_likelihood(
    ...
    for i, word in enumerate(words):
        total_log_likelihood[0,i] = calculate_log_likelihood(word,bigram_probabilities,char_to_index,start_token,end_token)
...
def sample_next_character(
    ...
    # TODO
    probs_for_char = probability_distribution[current_char_index]
    probs_for_char = torch.nan_to_num(probs_for_char, nan=0.0, posinf=1.0, neginf=0.0)


    # Sample an index from the distribution using the torch.multinomial function
    # TODO
    next_char_index: int = torch.multinomial(probs_for_char.float(), num_samples=1).item()
...
def calculate_log_likelihood(
    ...
    for i in range(len(word)-1):
        log_likelihood += torch.log(bigram_probabilities[char_to_index[word[i]]][char_to_index[word[i+1]]])
